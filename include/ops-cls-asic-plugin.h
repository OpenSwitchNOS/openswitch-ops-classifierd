...
enum ops_cls_lag_update_action
{
    ...
    OPS_CLS_LAG_MEMBER_INTF_DEL, /**< Remove a LAG member */
    OPS_CLS_MAX_ACTION
};
...
struct ops_cls_plugin_interface {
     ...
     * applied ACL in each direction. If any of the call fails, then
     * PI will perform a rollback of successfully applied list and
     * sets the incoming member interface to down. For general cases
     * such as apply ACL to a LAG or remove ACL from a LAG are
     * handled by apply and remove API respectively.
     *
     * All pointer arguments reference objects whose lifetimes are not
     * guaranteed to outlast this call. If you want to remember them for
     * later, you must make a copy.
