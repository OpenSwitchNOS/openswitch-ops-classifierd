...
struct ops_cls_plugin_interface {
     ...
     * in certain scenarios the incoming member interface is brought
     * down. For general cases such as apply ACL to a LAG or remove
     * ACL from a LAG are handled by apply and remove API
     * respectively.
     *
     * All pointer arguments reference objects whose lifetimes are not
     * guaranteed to outlast this call. If you want to remember them for
