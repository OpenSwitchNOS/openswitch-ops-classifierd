...
## Design choices
A key design principle for the ACL implementation is the need to be able to
fail the application of an ACL to an interface or fail the modification of an
applied ACL.  Another design principle is the need to handle multiple user
...
### ops-intfd
The application of ACLs in hardware will be used to set hw\_ready\_state column
in the interface table when ACLs has determined the interface is ready to pass
traffic. The ops-intfd daemon will monitor this column to determine if an
interface can be set to forward traffic.
...
- When a port is added to a LAG that has an ACL applied, if there are not
  ...
  port the hw\_ready\_state will be set to blocked for the associated interface.
...
### ops-classifierd
The classifierd daemon will update the hw_ready_state of the associated ports
in the interface table that have an ACL applied.
...
expires a list of all the ACLs and log enabled ACEs with their corresponding
hitcounts will be logged to the system log.  The next packet that matches a log
enabled ACE will be logged, repeating the process of starting the ACL logging
timer.
...
hardware to program the ACL on the physical interfaces.  Updates
aclv4\_in\_applied, aclv4\_in\_cfg, aclv4\_in\_cfg_version,
aclv4\_in\_status columns in the port table.
...
ACLs may be modified while the ACL is applied to ports.  Switchd will monitor
the ACL table's cfg_aces columns and update the in\_progress\_aces
...
### Interface Table
ACLs will set hw\_ready\_state column in the interface table when ACLs has
determined the interface is ready to pass traffic.
...
interface ACLs will set the hw\_ready\_state column to blocked.  The user must
unapply the ACL, free the needed hardware resources and then re-apply the ACL to
the LAG in order for all the interfaces within the LAG to have their
hw\_ready\_state columns set to forward.
