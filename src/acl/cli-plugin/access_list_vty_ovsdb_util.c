                              ...
                              const struct ovsrec_port *port_row)
{
    ...
    char *ace_str;
    int i;
    const struct ovsrec_acl* aclv4_applied = acl_db_util_get_applied(acl_db, port_row);
    ...
    /* Print each ACL entry as a single line (ala CLI input) */
    for (i = 0; i < aclv4_applied->n_cur_aces; i++) {
        /* If entry has or is a comment, print as its own line */
        if (aclv4_applied->value_cur_aces[i]->comment) {
            vty_out(vty,
                    "%20s  %" PRId64 " comment %s%s",
                    "",
                    aclv4_applied->key_cur_aces[i],
                    aclv4_applied->value_cur_aces[i]->comment,
                    VTY_NEWLINE);
        }
        if (aclv4_applied->value_cur_aces[i]->action) {
            if (aclv4_applied->value_cur_aces[i]->n_count) {
            ...
            ace_str = acl_entry_config_to_string(aclv4_applied->key_cur_aces[i],
                                                 aclv4_applied->value_cur_aces[i]);
            vty_out(vty, "  %s%s", ace_str, VTY_NEWLINE);
            free(ace_str);
        }
    }
