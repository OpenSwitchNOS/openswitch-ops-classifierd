...
cli_create_acl_if_needed(const char *acl_type, const char *acl_name)
{
    ...
    if (!acl_row) {
      ...
      if (ovs->n_acls >= max_acls_int) {
          vty_out(vty, "%% Unable to create ACL. Maximum allowed ACLs has been reached%s",
                  VTY_NEWLINE);
          cli_do_config_abort(transaction);
          return CMD_SUCCESS;
      } else {
    ...
    if (old_ace_row) {
      ...
      if (!ovs) {
          ...
          if (ace_entries > max_aces_int) {
              vty_out(vty, "%% Unable to create ACL entry. Maximum allowed ACEs has been reached%s",
                      VTY_NEWLINE);
              cli_do_config_abort(transaction);
              return CMD_SUCCESS;
          } else if (acl_row->n_cfg_aces >= max_aces_per_acl_int) {
              vty_out(vty, "%% Unable to create ACL entry. "
                      "Maximum allowed ACEs per acl has been reached%s",
                      VTY_NEWLINE);
              cli_do_config_abort(transaction);
              return CMD_SUCCESS;
          } else {
