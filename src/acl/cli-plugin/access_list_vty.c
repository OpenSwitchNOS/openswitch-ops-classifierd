    ...
    for (i = 0; i < acl_row->n_cur_aces; i++) {
        if (acl_row->key_cur_aces[i] == key) {
            ...
            if (value != NULL) {
                ...
                value_list[i] = value;
                entries_changed = 0;
            /* For delete operation, don't copy into cfg_aces */
            } else {
                entries_changed = -1;
            }
        } else {
            /* For all other entries, copy cur_aces to cfg_aces */
            key_list[i] = acl_row->key_cur_aces[i];
            value_list[i] = acl_row->value_cur_aces[i];
        }
    }
    /* If key was not found, it's not an update, or delete, so it's an insert.
       Append entry at end of list (will be sorted by key automatically). */
                             ...
                             char *ace_comment)
{
    ...
    ovsrec_acl_set_cfg_aces_from_cur_aces(acl_row, ace_sequence_number,
                                             (struct ovsrec_acl_entry *) ace_row);
    ovsrec_acl_set_cfg_version(acl_row, acl_row->cfg_version + 1);
    ...
    if (!strcmp(interface_type, "interface")) {
        ...
        if (!strcmp(acl_type, "ipv4") && !strcmp(direction, "in")) {
            ...
            if (port_row->n_aclv4_in_cfg_version) {
                new_cfg_version = port_row->aclv4_in_cfg_version[0] + 1;
                ovsrec_port_set_aclv4_in_cfg_version(port_row, &new_cfg_version, 1);
            } else {
                ovsrec_port_set_aclv4_in_cfg_version(port_row, 0, 1);
            }
        } else {
        ...
        if (!strcmp(acl_type, "ipv4") && !strcmp(direction, "in")) {
            ...
            if (vlan_row->n_aclv4_in_cfg_version) {
                new_cfg_version = vlan_row->aclv4_in_cfg_version[0] + 1;
                ovsrec_vlan_set_aclv4_in_cfg_version(vlan_row, &new_cfg_version, 1);
            } else {
                ovsrec_vlan_set_aclv4_in_cfg_version(vlan_row, 0, 1);
            }
        } else {
            vty_out(vty, "%% Unsupported ACL type or direction%s", VTY_NEWLINE);
            cli_do_config_abort(transaction);
