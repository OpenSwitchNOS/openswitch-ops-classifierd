...
#define PORT_NAME_STR_LEN                 16    /**< port name str length */
...
acl_update_internal(struct acl* acl)
{
    ...
    char port_name[PORT_NAME_STR_LEN];
    ...
    if (!list_is_empty(&acl->acl_port_map)) {
        ...
        if (rc == 0) {
            ...
            VLOG_DBG(details);
            snprintf(port_name,PORT_NAME_STR_LEN,"%d",status.port->ofp_port);
