...
/**************************************************************************//**
 * This function handles the LAG ifaces shutdown. It checks
 ...
 *****************************************************************************/
static void
acl_port_lag_ifaces_handle_shutdown(struct port *port,
                                    struct acl_port *acl_port,
                                    struct ofproto *ofproto)
{
    ...
    HMAP_FOR_EACH(port, hmap_node, &br->ports) {
        ...
        if (ACL_PORT_IS_LAG(port)) {
            ...
            if (port_iface_modified) {
                acl_port_lag_ifaces_handle_shutdown(port, acl_port,
                                                    br->ofproto);
            }
        }
    }
 ...
 *****************************************************************************/
void
acl_port_lag_ifaces_process_shutdown(struct blk_params *blk_params)
{
