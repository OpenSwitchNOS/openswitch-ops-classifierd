 ...
 *****************************************************************************/
static bool
acl_port_lag_iface_state_transitioned_or_added(struct iface *iface,
                                               struct acl_port *acl_port)
{
 ...
 *****************************************************************************/
static bool
acl_port_lag_iface_state_transitioned(struct iface *iface,
                                      struct acl_port *acl_port)
{
    struct acl_port_interface *acl_port_iface = NULL;
                                 ...
                                 struct ofproto *ofproto)
{
     ...
     */
    LIST_FOR_EACH(iface, port_elem, &port->ifaces) {
        if(acl_port_lag_iface_state_transitioned(iface, acl_port)) {
            acl_port_lag_check_and_set_members_active(acl_port);
                                   ...
                                   struct ofproto *ofproto)
{
    ...
    LIST_FOR_EACH(iface, port_elem, &port->ifaces) {
         ...
         */
        if(acl_port_lag_iface_state_transitioned_or_added(iface, acl_port)) {
