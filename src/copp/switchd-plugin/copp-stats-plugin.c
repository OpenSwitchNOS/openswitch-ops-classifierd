...
#define STATS_BUF_SIZE ((NUM_STATS_PER_CLASS*NUM_CHARS_UINT_64) + NUM_COMMAS_AND_SUCH)
    ...
    for (int tots=0; tots<COPP_STATS_TOTAL_MAX; tots++) {
        ...
        if (len < 0) {
            VLOG_WARN("could not convert totals to string. Not reporting "
                "total[%d]", tots);
            goto out;
        }
        if (len > STATS_BUF_SIZE) {
             VLOG_WARN("stringifying totals would over buffer space (should "
                "not happen). Not reporting total[%d]", tots);
            goto out;
        }
