...
uint64_t g_copp_stasts_totals[COPP_STATS_TOTAL_MAX] = {0,0,0,0};
...
void init(void) {
    ...
    rc = find_plugin_extension(COPP_ASIC_PLUGIN_INTERFACE_NAME,
        ...
        &asic_intf);
    if (rc) {
        VLOG_WARN("Could not find copp stats asic plugin. Will not register for switchd stats plugin");
        return;
    }

    /* register our callback on PER_BRIDGE. */
    rc = register_reconfigure_callback(copp_stats_brinit_cb, BLK_BRIDGE_INIT, 0);
    if (rc) {
        ...
        if (rc) {
            ...
            switch(rc) {
                ...
                case EINVAL:
                    ...
                    break;
                default:
                    VLOG_WARN("copp_hw_status_get() for class %d returned unrecognized %d %s", class, rc, strerror(rc));
            }
        }
