...
#if 0
/* Implementation of dummy finction that writes stats column in System table */
static int vtysh_ovsdb_system_statistics_column_set()
{
    const struct ovsrec_system *ovs_system = NULL;
    struct ovsdb_idl_txn *copp_auth_enable_txn = NULL;

    VLOG_INFO("Start transaction of writing to DB\n");

    /* Start of transaction */
    START_DB_TXN(copp_auth_enable_txn);

    /* Get access to the System Table */
    ovs_system = ovsrec_system_first(idl);
    if (NULL == ovs_system) {
        vty_out(vty, "Could not access the System Table during dummy write\n");
        return 1;
    }

    smap_replace((struct smap *)&ovs_system->copp_statistics,
                  SYSTEM_STATS_BGP_PKTS_PASSED, "1111");
    VLOG_INFO(" Wrote BGP\n");

    smap_replace((struct smap *)&ovs_system->copp_statistics,
                  SYSTEM_STATS_LLDP_PKTS_PASSED, "2222");
    VLOG_INFO(" Wrote LLDP\n");

    smap_replace((struct smap *)&ovs_system->copp_statistics,
                  SYSTEM_STATS_LACP_PKTS_PASSED, "3333");
    VLOG_INFO(" Wrote LACP\n");

    smap_replace((struct smap *)&ovs_system->copp_statistics,
                  SYSTEM_STATS_OSPFV2_PKTS_PASSED, "4444");
    VLOG_INFO(" Wrote OSPFV2\n");

    ovsrec_system_set_statistics(ovs_system, &ovs_system->copp_statistics);

    VLOG_INFO(" Successfully set_statistics column in System table\n");

    /* End of transaction. */
    END_DB_TXN(copp_auth_enable_txn);

    VLOG_INFO("Ending p  transaction of writing to DB\n");
...
static void tokenize(const char *buf)
{
    char *pch = NULL;
    char *str = NULL;
    int i = 1;
...
/* Implemenatation of the function which retrieves copp stats from OVSDB */
static void
...
#if 0
    /* Dummy code to be removed which writes to the DB in statistics column */
    rc = vtysh_ovsdb_system_statistics_column_set();
    if (rc) {
        VLOG_INFO("ERROR in writing to DB\n");
    } else {
        VLOG_INFO("SUCCESS in writing statistics ciolumn\n");
    }
#endif
...
vtysh_ovsdb_show_copp_generic_statistics()
{
    ...
    vty_out(vty, "\t  %s    %8s\t", "total_packets_passed:", ((buf) ? buf : COPP_ZERO_STRING));

    buf = smap_get(&ovs_system->copp_statistics,
                   SYSTEM_COPP_STATISTICS_MAP_TOTAL_BYTES_PASSED);
    vty_out(vty, "   %s    %8s\n", "total_bytes_passed:", ((buf) ? buf : COPP_ZERO_STRING));

    buf = smap_get(&ovs_system->copp_statistics,
                   SYSTEM_COPP_STATISTICS_MAP_TOTAL_PKTS_DROPPED);
    vty_out(vty, "\t  %s    %7s\t", "total_packets_dropped:", ((buf) ? buf : COPP_ZERO_STRING));

    buf = smap_get(&ovs_system->copp_statistics,
                   SYSTEM_COPP_STATISTICS_MAP_TOTAL_BYTES_DROPPED);
    vty_out(vty, "   %s    %7s\n\n\n", "total_bytes_dropped:", ((buf) ? buf : COPP_ZERO_STRING));
...
{
    /* ops-copp doesn't have any context level cli commands.
     * To load ops-copp cli shared libraries at runtime, this function is required.
     */
}
