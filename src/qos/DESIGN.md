...
The QoS features are implemented completely by the switching ASIC.  There are no daemons.  Openswitch provides the means for the user to configure and monitor the ASIC capabilities.
...
The primary purpose for QoS capabilities is to provide differentiated service to traffic - specially some traffic is treated preferentially over other kinds of traffic.  Queuing and scheduling are the fundamental capabilities of switching ASIC pipelines that provide differentiated service to traffic.
...
### ASIC QoS Meta-data
ASICs keep additional information about a packet, like its arrival port and VLAN, the intended destination port, etc.  There are two pieces of packet meta-data used by QoS used by ASICs to differentiate packets' handling and buffering.

**local-priority**
Generally is one of 8 levels (0-7) although some vendors use a 64 level encoding.
Zero is the lowest.  The allowed maximum will vary per ASIC family.
...
###Initial meta-data assignment (trust)
ASICs can be configured to inspect any VLAN priority tag or IP DSCP header of newly arrived packets to generate the initial values for packets' QoS meta-data.  By default these fields are ignored and all arriving traffic get the same QoS meta-data.  Administrators can configure the trust mode to 'cos' or 'dscp'  when the upstream sender  properly assign appropriate values to either the packets' CoS or DSCP fields.

Two global maps, cos-map and dscp-map, are used for the initial assignment.  The CoS priority or DSCP value is used as an index into the maps to retrieve the local-priority and color assignment for each packet.

### Queueing
As packets await transmission they are kept in separate queues depending on their local-priority meta-data.  Queues are numbered consecutively in priority order with zero being the lowest.  Queue profiles provide administrators to configure to which queue each local-priority will use.

### Scheduling
Scheduling is the process of selecting from which queue the next packet will be transmitted out the port.


## Key design choices
A key principle is that QoS capabilities and scale vary across ASIC families and vendors.  The design has to be flexible and extensible.  For example, factory defaults for the features are customized depending on the ASIC and product.
...
- QoS_DSCP_Map_Entry
	...
	- hw_config{} map also has keys that conain the factory default values
- QoS and Queue
	- Add two QoS rows for the default and factory-default schedule profiles.  The default profile can be modified by the customer, while factory-default is never modified or deleted.
		- The factory-default row has its hw_config boolen set to true.  This is used by the REST custom validator to prevent delete or modification operation on the row.
	- Add children Queue rows for the default and factory-default profiles
		- The factory-default Queue child rows has their hw_config boolen set to true. This is used by the REST custom validator to prevent delete or modification operation on the row.
- Q_Profile and Q_Profile_Entry
	...
	- Add children Q_Profile_Entry rows for the default and factory-default profiles
		- The factory-default Q_Profile_Entry child rows has their hw_config boolen set to true. This is used by the REST custom validator to prevent delete or modification operation on the row.
...
- Check the transaction against the same acceptance rules as CLI changes
...
When SWITCHD spawns, its plug-in infra will load the QoS plug-in.  During QoS Plug-in initialization it will:
-  Call the ASIC-Specific Provider to find the QoS APIs and verifies their version is supported by the QOS plug-in
-  Register with SWITCHD plug-in infra for callbacks for bridge and port changes
-  *?Register with OVSDB IDL for notifications for QoS tables?*

#### Queue statistics collection
The code that publishes per-queue statistics in the Interface table is not part of the QoS Plug-in.  Since it uses a preexisting Open vSwitch netdev API, dump_queue_stats(), the code is embedded directly into platform-independent SWITCHD.

SWIICHD periodically queries every interface (i.e. every 5 seconds by default) the provider queue statistics netdev-provider statistic API, dump_queue_stats(), and publishes the counts in the Interface row.

### ASIC-Specific Provider
These QoS APIs must be implemented from the following headers:
- qos-provider.h
	- qos_cos_map_set()
	- qos_dscp_map_set()
	- qos_port_config_set()
...
The preexisting netdev-provider QoS and Queue APIs are not used as they are more oriented to Linux qdisc and do not have the needed flexibility/extensiblity.  They are also per-interface without global QoS settings.

The QoS API follows a flexible & extensible pattern:
- configure a capability only globally as well as configure a capability per-port
...
For example, the QoS API CoS and DSCP MAP APIs.  Currently only configuring a single global map is supported.  Still a bundle pointer parameter is included to be forward compatible with ASICs that may implement a CoS or DSCP Map per port.  Also, the other_config key-value hash map is also a parameter.


## Structure View
The following repositories have QoS specific implementations:
```ditaa
  ops-cli                       ops-hw-config
 +----------------+            +-------------+
 |Profile contexts|            |qos.yaml file|
 +----------------+            +-------------+
 ...
 +--------------+         +------------------------+
  ...
  ops-classifierd/QOS                ops
+-----------------------+      +-------------+
| Command Lines         |      |Schema       |
|    show running-config|      |CLI Manuals  |
| Component Tests       |      +-------------+
| REST Custom Validators|
| QoS SWITCHD Plug-in   |        ops-switchd
+-----------------------+      +-------------+
                               |Plug-in infra|
                               |Queue Stats  |
                               +-------------+
...
- All command line (VTYSH) functions including distributed a component of 'show running-config'
- All REST Custom Validators
- CLI and REST Component tests
	- These are written to the existing VSI framework
	- Converstion to the new TOX framework will occur afer Dill PSI.
- QoS SWITCHD plug-in
...
**ops-hw-config**
THis repository has the qos.yaml files for each target platform.

**ops-intfd**
QoS additions to the 'show interface' and 'show runnig-config interface'.

**ops-switchd**
The plug-in infrasturcture is configured, via its YAML file, to load the QoS Plug-in.  Also, the calls to netdev dump_queue_state() are embedded direcly in bridge.c and subsystem.c files.
...
- **Q_Profile** and **Q_Profile_Entry**
	...
	- There is a special  'factory-default' profile with the hw_default column set to true
		...
		- REST custom validators will not allow this profile and its entries form being modified or deleted
- **QoS** and **Queue**
...
- **Port** for per-port configuration
	...
	- qos_config has keys:
		- Name of the last successfully applied default queue profile
		- Name of the last successfully applied default schedule profil
...
The four new APIs are declared in qos-asic-provider.h.  There are implemented as an optional extension API.  ASIC-Specififc Provider plug-ins can choose not to include the entire API set, or only implement a subset of the functions.
...
```
The get_queue_stats() function in netdev-provider.h is called periodically for each interface to publish the per-queue statistics into the coresponding Interface table row map columns:
...
Caller supplies netdev_queue_stats buffer for the ASIC-Specific Provider to fill.  Any unsupported counts should be will with all one bits (UINT64_MAX).
