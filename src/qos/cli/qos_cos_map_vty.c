...
DEFUN (qos_cos_map,
    ...
    strncpy(aubuf, "op=CLI: qos cos-map", QOS_CLI_AUDIT_BUFFER_SIZE);
...
static int qos_cos_map_no_command(int64_t code_point) {
    ...
    int64_t local_priority = atoi(smap_get(&cos_map_row->hw_defaults, QOS_DEFAULT_LOCAL_PRIORITY_KEY));
    const char *color = smap_get(&cos_map_row->hw_defaults, QOS_DEFAULT_COLOR_KEY);
    const char *description = smap_get(&cos_map_row->hw_defaults, QOS_DEFAULT_DESCRIPTION_KEY);
...
DEFUN (qos_cos_map_no,
        ...
        "The QoS COS Map name\n") {
    char aubuf[QOS_CLI_AUDIT_BUFFER_SIZE];
    strncpy(aubuf, "op=CLI: no qos cos-map", QOS_CLI_AUDIT_BUFFER_SIZE);
    char hostname[HOST_NAME_MAX+1];
    gethostname(hostname, HOST_NAME_MAX);
    int audit_fd = audit_open();
...
static void print_cos_map_row(struct ovsrec_qos_cos_map_entry *cos_map_row,
    ...
    int code_point = is_default ?
            atoi(smap_get(&cos_map_row->hw_defaults, QOS_DEFAULT_CODE_POINT_KEY)) :
            (int) cos_map_row->code_point;
    vty_out (vty, "%-10d ", code_point);

    int local_priority = is_default ?
            atoi(smap_get(&cos_map_row->hw_defaults, QOS_DEFAULT_LOCAL_PRIORITY_KEY)) :
...
static vtysh_ret_val qos_cos_map_show_running_config_callback(
        void *p_private) {
    ...
    OVSREC_QOS_COS_MAP_ENTRY_FOR_EACH(cos_map_row, idl) {
        ...
        const char *default_color =
                smap_get(&cos_map_row->hw_defaults, QOS_DEFAULT_COLOR_KEY);
        if (strncmp(cos_map_row->color, default_color, QOS_CLI_MAX_STRING_LENGTH) != 0) {
            display_headers(&cos_map_header_displayed,
                    &code_point_header_displayed, code_point);
        ...
        const char *default_description =
                smap_get(&cos_map_row->hw_defaults, QOS_DEFAULT_DESCRIPTION_KEY);
        if (strncmp(cos_map_row->description, default_description, QOS_CLI_MAX_STRING_LENGTH) != 0) {
            char description[QOS_CLI_STRING_BUFFER_SIZE];
            if (strncmp(cos_map_row->description, "", QOS_CLI_MAX_STRING_LENGTH) == 0) {
                strncpy(description, QOS_CLI_EMPTY_DISPLAY_STRING,
                        QOS_CLI_EMPTY_DISPLAY_STRING_BUFFER_SIZE);
            } else {
