...
classifierd_ovsdb_init(const char *db_path)
{
    ...
    /* SB: we need column to monitor per interface ACL applied status
    ...
    /* TODO: update for security_state */
...
intf_parse_security(const struct ovsrec_interface *intf_row) {
...
add_new_interface(const struct ovsrec_interface *ifrow)
{
    ...
    new_intf->port_admin = intf_parse_security(ifrow);
...
remove_interface_from_port(const struct ovsrec_port *port_row)
{
    ...
    if (!port_row->n_interfaces) {
        ...
        for(j = 0; j < port_data->n_interfaces; j++) {
            ...
            intf = shash_find_data(&all_interfaces, intf_row->name);
            if(!intf)
		VLOG_DBG("intf is NULL\n");

#if 0
            if (port_parse_admin(&intf->port_admin, intf_row)) {
                intf->user_cfg.admin_state = intf_parse_security(intf_row);
...
classifierd_reconfigure(void)
{
   ...
   // const struct ovsrec_subsystem *subrow = NULL;
    ...
    VLOG_DBG("classifierd_reconfigure\n");
