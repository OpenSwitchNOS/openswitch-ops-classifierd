...
{
    if(interface == NULL) {
        VLOG_ERR("classifierd_debug_dump_interface: NULL interface\n");
        return;
    }
    ...
    OVSREC_PORT_FOR_EACH (port_row, idl) {
        if (list_all_ports
		    ...
		    if(port_row->n_interfaces == 1) {
                ...
                classifierd_debug_dump_port_acl_info(ds,port_row);
				for(iface_idx = 0; iface_idx < port_row->n_interfaces; iface_idx++) {
...
{
    VLOG_DBG("ovsdb_init at db_path = %s\n", db_path);
...
classifierd_reconfigure(void)
{
    ...
    if (new_idl_seqno == idl_seqno) {
        VLOG_DBG("classifierd_reconfigure, no change in the db\n");
...
classifierd_system_is_configured(void)
{
    ...
    if (sysrow && sysrow->cur_cfg > INT64_C(0)) {
        ...
        VLOG_DBG("System (cur_cfg=%" PRId64 ").",
                 sysrow->cur_cfg);
    }
...
classifierd_run(void)
{
    ...
    VLOG_DBG("Process a batch of messages from OVSDB \n");
    ovsdb_idl_run(idl);
    ...
    if (!classifierd_system_is_configured()) {
        VLOG_INFO_ONCE("System is NOT configured yet, nothing to do...\n");
        return;
    }
