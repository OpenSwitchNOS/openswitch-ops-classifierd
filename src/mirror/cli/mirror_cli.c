...
source_iface_exec(const char* iface_name, const char* direction, bool delete)
{
   ...
   if (delete) {
      ...
      if ((rc == CMD_SUCCESS) && (is_port_in_src_set(port_row))) {
         rc = update_src_port (port_row, true);
      }

      if ((rc == CMD_SUCCESS) && (is_port_in_dst_set(port_row))) {
         rc = update_dst_port (port_row, true);
      }
      ...
      if ((strcmp(direction, SRC_DIR_TX) == 0) ||
         ...
         if ((rc == CMD_SUCCESS) && (!is_port_in_dst_set(port_row))) {
            rc = update_dst_port (port_row, false);
         }
      }
      ...
      if ((strcmp(direction, SRC_DIR_RX) == 0) ||
         ...
         if ((rc == CMD_SUCCESS) && (!is_port_in_src_set(port_row))) {
            rc = update_src_port (port_row, false);
         }
      }
   }
