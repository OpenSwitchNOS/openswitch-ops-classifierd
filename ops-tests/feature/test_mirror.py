...
def setup_topo(topology):
    ...
    eth = sn1.ports['1']
    print("sniffer port " + eth)

    # sn1.send_command('apt-get install scapy', shell='bash')
...
%(levelname)s - %(message)s')
    ...
    assert not ereq, "Expect zero pings!"
    assert not erply, "Expect zero pings!"
    # set_trace()
    ...
    print("##################################################")
    # Continuous ping (H1->H2) only
    hs1.send_command("(ping -q -i 0.5 10.0.10.2) &", shell='bash')
    # ---------------------------------------
    ...
    # ---------------------------------------
    print("##################################################")
    print(" CASE 2.1 Verify Sniffer Flow Egress")
    print("##################################################")
    logger.info('========= CASE 2.1 -  src I/F 1 rx, dest I/F 4  =========')
    ...
    logger.info('==== END CASE 2.1 ====')
    #  Verify Flow for  REQ only
    logger.info(sn1.send_command('echo "sniff(iface=\\"' + eth + '\\", '
                                 'prn=lambda x: x.summary(),'
                                 'timeout=5)" | scapy 2>/dev/null',
                                 shell='bash'))

    # Stop the ping
    ereq, erply = summary.ping_count("CASE 2.1", "END CASE 2.1")
    logger.info("PING-REQUESTS " + str(ereq))
    logger.info("PING-REPLIES " + str(erply))
    if (erply is not 0):
        assert False, "ICMP should only be echo-requests"

    print("##################################################")
    print(" CASE 2.2 Verify Sniffer Flow Ingress")
    print("##################################################")
    logger.info('========= CASE 2.2 -  src I/F 1 rx, dest I/F 4  =========')
    # Turn off H1 ping
    hs1.send_command("pkill ping", shell='bash')
    # Turn on H3->H1 ping
    hs3.send_command("(ping -q -i 0.5 10.0.10.1) &", shell='bash')
    # Verify Flow for REPLY only
    logger.info(sn1.send_command('echo "sniff(iface=\\"' + eth + '\\", '
                                 'prn=lambda x: x.summary(),'
                                 'timeout=5)" | scapy 2>/dev/null',
                                 shell='bash'))
    # Stop the ping
    logger.info('==== END CASE 2.2 ====')
    ereq, erply = summary.ping_count("CASE 2.2", "END CASE 2.2")
    logger.info("PING-REQUESTS " + str(ereq))
    logger.info("PING-REPLIES " + str(erply))
    if (ereq is not 0):
        assert False, "ICMP should only be echo-replies"
    # Turn off H3 ping
    hs3.send_command("pkill ping", shell='bash')
    # set_trace()
    ...
    for listentry in responselist:
        ...
        if "echo-reply" in listentry:
            if '10.0.10.3 > 10.0.10.1' in listentry:
                passed = True
            else:
                assert False, "bad packet"
        if "echo-requests" in listentry:
            assert False, "bad packet"
    print(passed)
    ...
    assert not ops1('end', shell='vtysh')
    vlan_result = ops1('show vlan 100')
    # Test pings AFTER mirror (expect zero on sniffer node)
    responselist = pingandsniff(0, topology)
    for listentry in responselist:
        logger.info(listentry)
        if "echo-reply" in listentry:
