...
def test_traffic_on_port(topology, step):
    ...
    # t4snifficmpcountpacket, sendscapypacket
    # snifficmpcountpacket, sendpcountpacket cnt 10
    txthread = ScapyMultiCountThread(
                functions[0],
                'hs1', topology, proto_str, list1, functions[0].__name__)
    rxthread = ScapyMultiCountThread(
    ...
    assert search(
       ''
       r'7\s+permit\s+icmp\s+10\.0\.10\.1/255\.10\.255\.0'
       '\s+10\.0\.10\.2'.format(
                                         **locals()
                                       ), test1_result
    )
    ...
    assert search(
       ''
       r'8\s+permit\s+icmp\s+10\.[0-9]\.10\.1/255\.0\.0\.0'
