...
def test_acl_on_lag_with_members(configure_lag_l3, topology, step):
    ...
    configure_acl_l3(
                    ops2, 'ip', test_acl_name, '10', 'permit', 'icmp', 'any',
                    '', 'any', '', '', ""
                )
    ...
    # tear down
    teardown_topology(ops1, interface_list, '', '', lag_id_s1)
    teardown_topology(ops2, interface_list, 'ip', test_acl_name, lag_id_s2)
...
def test_acl_on_lag_with_one_member(configure_lag_l3, topology, step):
    ...
    # tear down
    teardown_topology(ops1, [interface_list[1]], '', '', lag_id_s1)
    teardown_topology(ops2, [interface_list[1]], 'ip', test_acl_name,
                      lag_id_s2)
...
def test_acl_on_lag_with_no_members(configure_lag_l3, topology, step):
    ...
    # tear down
    teardown_topology(ops1, [], '', '', lag_id_s1)
    teardown_topology(ops2, [], 'ip', test_acl_name, lag_id_s2)
...
@mark.platform_incompatible(['docker'])
def test_acl_on_lag_with_shut_on_all_members(configure_lag_l3,
                                             topology, step):
...
def test_lag_with_no_member_apply_acl(configure_lag_l3, topology, step):
    ...
    # apply acl on lag for switch 2
    apply_acl(
        ops2, 'lag', str(lag_id_s2), 'ip', test_acl_name, 'in',
...
def test_bring_down_lag_members_using_pspo_infra(
    ...
    apply_acl(ops2, 'port', 'if01', 'ip', max_test_acl_name, 'in')
    sleep(6)
    """
    LAG has no members, so no traffic is sent through the
    switch in both the direction
    ...
    unapply_acl(ops2, 'port', 'if01', 'ip', max_test_acl_name, 'in')
    sleep(5)
    unapply_acl(ops2, 'lag', str(lag_id_s2), 'ip', test_acl_name, 'in')
    sleep(5)

    # tear down
    unconfigure_acl(ops2, 'ip', max_test_acl_name)
    teardown_topology(ops1, interface_list, '', '', lag_id_s1)
    teardown_topology(ops2, [interface_list[0]], 'ip', test_acl_name,
                      lag_id_s2)

    step("*****END OF TEST CASE 9*******")


@mark.platform_incompatible(['docker', 'ostl'])
...
def test_replace_acl_when_lag_is_shutdown(configure_lag_l3, topology, step):
    ...
    configure_acl_l3(
                    ops2, 'ip', test_acl_name, '10', 'permit', 'udp', 'any',
                    '', 'any', '', '', ""
                )
...
@mark.platform_incompatible(['docker', 'ostl'])
def test_egress_acl_on_lag_with_members(configure_lag_l3,
                                        topology, step):
    ...
    # apply acl on lag for switch 2
    apply_acl(ops2, 'lag', str(lag_id_s2), 'ip', test_acl_name, 'out')
...
@mark.platform_incompatible(['docker', 'ostl'])
def test_egress_acl_on_lag_with_one_member(configure_lag_l3,
                                           topology, step):
...
@mark.platform_incompatible(['docker', 'ostl'])
def test_egress_acl_on_lag_with_no_member(configure_lag_l3,
                                          topology, step):
...
@mark.platform_incompatible(['docker', 'ostl'])
def test_egress_acl_on_lag_with_shut_on_lag(configure_lag_l3,
                                            topology, step):
...
@mark.platform_incompatible(['docker', 'ostl'])
def test_egress_lag_with_no_member_apply_acl(configure_lag_l3,
                                             topology, step):
    ...
    # apply acl on lag for switch 2
    apply_acl(
        ops2, 'lag', str(lag_id_s2), 'ip', test_acl_name, 'out',
        suppress_warning=True
        )
    # tear down
    teardown_topology(ops1, interface_list, '', '', lag_id_s1)
...
@mark.platform_incompatible(['docker', 'ostl'])
def test_egress_config_persistence(configure_lag_l3,
                                   topology, step):
...
def test_lag_members_brought_up_through_pspo_infra_by_removing_acl(
    ...
    unconfigure_acl(ops2, 'ip', max_test_acl_name)
    teardown_topology(ops1, interface_list, '', '', lag_id_s1)
    teardown_topology(ops2, [interface_list[0]], '', '', lag_id_s2)
...
def test_config_persistence_lag_members_are_down_by_pspo_infra(
    ...
    # removing all members from lag switch2
    update_lag_members(ops2, interface_list, lag_id_s2, False)
    sleep(5)
    # checking ovsdb status in hardware
    verify_interface_hw_status(ops2, 0)
...
def teardown_topology(ops, lag_interface_list, acl_type, acl_name, lag_id):
