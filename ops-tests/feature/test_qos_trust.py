...
# ------------------------------------------------------------
# Parse trust local priority override
# ------------------------------------------------------------
def parselp(f1, patt):
    out = []
    buff = []
...
def setup_topo1(topology):
    ...
    # Configure ops OOBM port for debugging while test is paused
    ops1('configure terminal', shell='vtysh')
    ops1('interface mgmt', shell='vtysh')
    ops1('ip static 10.0.0.2/24', shell='vtysh')
    ops1('end', shell='vtysh')
...
%(levelname)s - %(message)s')
    ...
    assert not ops1('configure terminal', shell='vtysh')
    assert not ops1('qos cos-map 0 local-priority 1', shell='vtysh')
    assert not ops1('qos cos-map 1 local-priority 0', shell='vtysh')
    for cp in range(2, 7):
        assert not ops1('qos cos-map ' + str(cp) +
                        ' local-priority ' + str(cp),
                        shell='vtysh')
    assert not ops1('end', shell='vtysh')
    cosmap = ops1('show qos cos-map', shell='vtysh')
    cosmaparray = parse_cp(cosmap)
    ...
    # Trust override
    rconfig = ops1('show running-config')
    local_pri = parselp(rconfig, 'qos dscp-map 26')
