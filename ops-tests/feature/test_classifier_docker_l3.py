...
def test_lag_create_add_apply(topology):
    ...
    # Apply ACL
    apply_acl(ops2, 'lag', str(lag_id_s1), 'ip', acl_name, 'in')
...
def test_lag_create_add_apply_delete_lag(topology):
    ...
    config_lag(ops2, lag_id_s1, enable=False)
...
def test_lag_create_apply_add_one_member(topology):
    ...
    update_lag_members(ops2, [interface_list[0]], lag_id_s1, True)
...
# 9
def test_lag_create_apply(topology):
    """
...
# 10
def test_lag_create_add_member_after_apply(topology):
    """
    Create a lag, Apply ACL to lag,
    ...
    ops1 = topology.get('ops1')
    ops2 = topology.get('ops2')
    # Configure the switch - Create a lag, do not add interfaces,
    #               configure routes and assign ip address to lag
    #               and interface 1
    config_switches_l3_lag(ops1, ops2, ip_ops1_int1, ip_ops2_int1,
                           ip_ops1_lag, ip_ops2_lag, ip_route_ops1,
                           ip_route_ops2, lag_id_s1, lag_id_s2, [])
    ...
    update_lag_members(ops2, interface_list, lag_id_s1, True)
...
# 13
@pytest.mark.test101
def test_lag_create_add_member_shut_lag_replace_acl(topology):
    """
    Create a lag, add members and apply ACL to lag,
    shut lag, replace ACL
    no shut lag, the change should be seen
    ...
    apply_acl(ops2, 'lag', str(lag_id_s1), 'ip', 'newACL', 'in')
    # No Shut LAG
    config_interface_state(ops1, 'lag', [lag_id_s1], True)
    config_interface_state(ops2, 'lag', [lag_id_s1], True)
    # Verify now acl entry is applied to lag interfaces
