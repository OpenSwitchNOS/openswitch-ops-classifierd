...
def no_shut_route_interfaces_lag(switch, iface_1, iface_2, iface_3):
    no_shut_interface(switch, iface_1)
    no_routing_interface(switch, iface_1)
    no_shut_interface(switch, iface_2)
    no_shut_interface(switch, iface_3)


def interface_up_lag(switch, iface1, iface2, iface3):
    for ops, portlbl in [(switch, iface1), (switch, iface2), (switch, iface3)]:
        wait_until_interface_up(ops, portlbl)


def apply_acl_on_lag_in(ops, lag_id, acl_name):

    with ops.libs.vtysh.ConfigInterfaceLag(lag_id) as ctx:
        ctx.apply_access_list_ip_in(acl_name)


def apply_acl_on_lag_out(ops, lag_id, acl_name):

    with ops.libs.vtysh.ConfigInterfaceLag(lag_id) as ctx:
        ctx.apply_access_list_ip_out(acl_name)


def check_lag_applied_to_member(lag_member, lag_id, run_result):
    """
    for use in show run
    """
    lag_member_id_str = 'interface {}.*'.format(lag_member)
    lag_member_section = re.findall(
            lag_member_id_str, run_result, re.DOTALL
        )
    print('lag_member_section is {}'.format(lag_member_section))
    print('skipped first line: {}'.format(
                lag_member_section[0].splitlines()[2]))
    print('lag_id is {}'.format(lag_id))
    assert(str(lag_id) == search(
            '(?<=lag )\d+', lag_member_section[0].splitlines()[2]).group())


def apply_no_lag_on_interface(ops, lag_id, interface):

    with ops.libs.vtysh.ConfigInterface(interface) as ctx:
        ctx.no_lag(lag_id)


def add_lag_to_vlan(ops, lag_id, vlan_id):
...
def no_vlan(ops, lag_id, vlan_id):

    with ops.libs.vtysh.ConfigInterfaceLag(lag_id) as ctx:
        ctx.no_vlan_access(vlan_id)


def shut_interface(ops, interface):

    with ops.libs.vtysh.ConfigInterface(interface) as ctx:
        ctx.shutdown()


def no_shut_interface(ops, interface):

    with ops.libs.vtysh.ConfigInterface(interface) as ctx:
        ctx.no_shutdown()


def no_routing_interface(ops, interface):

    with ops.libs.vtysh.ConfigInterface(interface) as ctx:
        ctx.no_routing()


def create_lag(ops, lag_id):

    with ops.libs.vtysh.ConfigInterfaceLag(lag_id) as ctx:
        ctx.no_shutdown()


def shut_lag(ops, lag_id):

    with ops.libs.vtysh.ConfigInterfaceLag(lag_id) as ctx:
        ctx.shutdown()


def no_shut_lag(ops, lag_id):

    with ops.libs.vtysh.ConfigInterfaceLag(lag_id) as ctx:
        ctx.no_shutdown()


def apply_members_to_lag(ops, interface, lag_id):

    with ops.libs.vtysh.ConfigInterface(interface) as ctx:
        ctx.lag(lag_id)


def remove_members_from_lag(ops, interface, lag_id):

    with ops.libs.vtysh.ConfigInterface(interface) as ctx:
        ctx.no_lag(lag_id)


def delete_lag(ops, lag_id):
...
def verify_appctl_acl_applied(
                switch, acl_name, lag_member, direction, acl_expect
                    ):
    """
    ...
    if lag_member != 0:
        assert(
            acl_expect == (
                port_acl_dir.group(1) == switch.ports[lag_member] and
                port_acl_dir.group(3) == direction
            )
        )
    """
