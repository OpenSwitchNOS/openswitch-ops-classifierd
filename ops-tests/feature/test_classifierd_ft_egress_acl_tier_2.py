...
Test1 : test_acl_udp_any_any_permit
Test2 : test_acl_udp_any_any_deny
Test3 : test_acl_permit_udp_hs1_hs2
Test4 : test_acl_deny_udp_hs1_hs2
Test5 : test_acl_permit_udp_prefix_len_mask
Test6 : test_acl_deny_udp_prefix_len_mask
Test7 : test_acl_permit_udp_dotted_netmask
Test8 : test_acl_deny_udp_dotted_netmask
Test9 : test_acl_permit_udp_non_contiguous_mask
Test10: test_acl_deny_udp_non_contiguous_mask
Test11: test_acl_permit_udp_dport_eq_param
Test12: test_acl_deny_udp_dport_eq_param
Test13: test_acl_deny_udp_dport_eq_param
Test14: test_acl_deny_udp_dport_eq_param
Test15: test_acl_modify_after_sending_udp_traffic
Test16: test_acl_deny_udp_on_multiple_ports
Test17: test_acl_permit_icmp_on_multiple_ports
Test18: test_acl_replace_with_icmp_traffic
"""
...
def test_acl_permit_udp_any_any(configure_acl_test, topology, step):
    """
    This test adds a "1 permit udp any any" rule on interface 2.
    ...
    step('1.b Apply and egress ACL to port')
    # with ops1.libs.vtysh.ConfigInterface('1') as ctx:
        # ctx.apply_access_list_ip_in('test')
    with ops1.libs.vtysh.ConfigInterface('2') as ctx:
        ctx.apply_access_list_ip_out('test')

    test1_result = ops1('show run')

    assert search(
        # r'(access-list\s+ip\s+test\s+\in)'.format(
        r'(access-list\s+ip\s+test\s+\out)'.format(
                                          **locals()
                                        ), test1_result
    )

    step('1.c Create UDP packets')
    ...
    step('1.d Send and receive udp packets on hs1 and hs2')
    rxthread.start()
    txthread.start()
...
def test_acl_deny_udp_any_any(configure_acl_test, topology, step):
    """
    This test adds a "1 deny udp any any" rule on interface 2.
    It then sends 10 UDP packets from hs1 to hs2 and verifies that
    10 UDP packets are received on hs2
    """
    global filter_udp, timeout, count, port_str
...
def test_acl_permit_udp_hs1_hs2(configure_acl_test, topology, step):
    """
    This test adds a "1 permit udp 1.1.1.1 1.1.1.2" rule on interface 2.
    It then sends 10 UDP packets from hs1 to hs2 and verifies that
...
def test_acl_deny_udp_hs1_hs2(configure_acl_test, topology, step):
    """
    This test adds a "1 deny udp 1.1.1.1 1.1.1.2" rule on interface 2.
...
def test_acl_permit_udp_prefix_len_mask(configure_acl_test, topology, step):
    """
    This test adds a "1 permit udp 1.1.1.0/31 1.1.1.0/30" rule on interface 2.
    It then passes 10 UDP packets from hs1 to hs2 and verifies that
    10 UDP packets are received on hs2. Also, it verifies that other
    protocol traffic is not received by hs2 by sending 10 ICMP packets.
    """
...
def test_acl_permit_udp_dotted_netmask(configure_acl_test, topology, step):
    ...
    1.1.1.0/255.255.255.252" rule on interface 2.
    It then passes 10 UDP packets from hs1 to hs2 and verifies that
    10 UDP packets are received on hs2. Also, it verifies that other
    protocol traffic is not received by hs2 by sending 10 ICMP packets.
    """
...
def test_acl_modify_after_sending_udp_traffic(
                                        configure_acl_test, topology, step):
    """
    This test sends some traffic after applying an ACL to interface 2.
...
def test_acl_permit_icmp_on_multiple_ports(configure_acl_test, topology, step):
    ...
    ACL to interfaces 1 and 2. After it verifies that hs2 receives 10 packets,
...
def test_acl_replace_with_icmp_traffic(configure_acl_test, topology, step):
    ...
    on interface 2. Verifies that the packets have been received on hs2.
    ...
    with ops1.libs.vtysh.ConfigInterface('2') as ctx:
        ctx.apply_access_list_ip_out('test2')

    test1_result = ops1('show run')

    assert search(
        # r'(access-list\s+ip\s+test2\s+\in)'.format(
        r'(access-list\s+ip\s+test2\s+\out)'.format(
                                          **locals()
                                        ), test1_result
    )
