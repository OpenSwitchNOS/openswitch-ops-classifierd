...
@mark.platform_incompatible(['docker'])
def test_acl_on_lag_with_shut_on_all_members(configure_lag_l2, topology, step):
...
@mark.platform_incompatible(['docker'])
def test_egress_acl_on_lag_with_members(configure_lag_l2, topology, step):
...
@mark.platform_incompatible(['docker'])
def test_egress_acl_on_lag_with_one_member(configure_lag_l2, topology, step):
...
@mark.platform_incompatible(['docker'])
def test_egress_acl_on_lag_with_no_member(configure_lag_l2, topology, step):
...
@mark.platform_incompatible(['docker'])
def test_egress_acl_on_lag_with_shut_on_lag(configure_lag_l2, topology, step):
...
@mark.platform_incompatible(['docker'])
def test_egress_lag_with_no_member_apply_acl(configure_lag_l2, topology, step):
...
@mark.platform_incompatible(['docker'])
def test_egress_config_persistence(configure_lag_l2, topology, step):
...
def test_lag_members_brought_up_through_pspo_infra_by_removing_from_lag(
    ...
    # removing one memeber from lag of switch2
    update_lag_members(ops2, [interface_list[0]], lag_id_s1, False)
    # checking ovsdb status in hardware
