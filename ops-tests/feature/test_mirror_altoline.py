...
def test_mirror_routing(topology):
    ...
    which_test = 'routing'
    global ran_one_test
    if(ran_one_test is True):
        sleep(20)
    ops, hs1, hs2, sn1 = setup_topology(topology, 'hw', ran_one_test)
    if(ran_one_test is True):
        sleep(20)
    run_mirror_test(ops, hs1, hs2, sn1)
    teardown_hosts(hs1, hs2, sn1)
    if(ran_one_test is False):
        sleep(15)
    ran_one_test = True
...
def test_mirror_vlan_docker(topology):
    ...
    global wait_time
    # if ran on docker wait for 10 secs after changing mirrors
    # because docker takes a while to stabalize
    wait_time = 10
    global which_test
    which_test = 'vlan'
    global ran_one_test
    ops, hs1, hs2, sn1 = setup_topology(topology, 'docker', ran_one_test)
    sleep(wait_time)
    run_mirror_test(ops, hs1, hs2, sn1)
...
def setup_topology(topology, platform, ran_one_test):
    ...
    if(ran_one_test is True):
        ops.libs.reboot.reboot_switch(shell="vtysh")
    sleep(5)
    ...
    if(which_test == 'vlan'):
        ...
        hs2_ip_info['ip'] = '10.10.11.1'
        hs2_ip_info['ip_mask'] = '10.10.11.1/24'
...
def teardown_hosts(hs1, hs2, hs3):
    hs1.send_command('ip addr flush dev eth1')
    hs2.send_command('ip addr flush dev eth1')
    hs3.send_command('ip addr flush dev eth1')
