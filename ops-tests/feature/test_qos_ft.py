...
def case_qosApplyPortShow():
    ...
    assert 'schedule-profile strict' in out
    ...
    out = ops1(format('show running-config interface lag10'))
    assert 'schedule-profile default' in out
    ...
    out = ops1(format('show running-config interface {p1}'))
    assert 'schedule-profile' not in out
    out = ops1.libs.vtysh.show_interface(p1)
    assert out['qos_schedule_profile'] == 'default'
