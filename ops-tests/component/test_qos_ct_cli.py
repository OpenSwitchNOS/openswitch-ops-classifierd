...
def setUp_qosApplyGlobal():
    with ops1.libs.vtysh.ConfigQueueProfile('p1') as ctx:
    ...
    with ops1.libs.vtysh.ConfigScheduleProfile('p1') as ctx:
...
def case_qosApplyGlobalCommand():
    with ops1.libs.vtysh.Configure() as ctx:
        ctx.apply_qos_queue_profile_schedule_profile('p1', 'p1')

    out = ops1.libs.vtysh.show_qos_queue_profile()
    assert out['p1']['profile_status'] == 'applied'
    assert out['p1']['profile_name'] == 'p1'
...
def case_qosApplyGlobalCommandWithMissingScheduleProfile():
    with raises(TcamResourcesException):
        with ops1.libs.vtysh.Configure() as ctx:
            ctx.apply_qos_queue_profile_schedule_profile('p1', 'missing')
...
def case_qosApplyGlobalCommandWithAllStrict():
    with ops1.libs.vtysh.Configure() as ctx:
        ctx.apply_qos_queue_profile_schedule_profile('p1', 'AllStrict')
...
def case_qosApplyGlobalCommandWithAllWrr():
    with ops1.libs.vtysh.Configure() as ctx:
        ctx.apply_qos_queue_profile_schedule_profile('p1', 'AllWrr')
...
def case_qosApplyGlobalCommandWithAllWrrWithMaxStrict():
    with ops1.libs.vtysh.Configure() as ctx:
        ctx.apply_qos_queue_profile_schedule_profile(
            'p1', 'AllWrrWithMaxStrict')

    out = ops1.libs.vtysh.show_qos_queue_profile()
    assert out['p1']['profile_status'] == 'applied'
    assert out['p1']['profile_name'] == 'p1'
    out = ops1.libs.vtysh.show_qos_schedule_profile()
    assert out['AllWrrWithMaxStrict']['profile_status'] == 'applied'
    assert out['AllWrrWithMaxStrict']['profile_name'] == 'AllWrrWithMaxStrict'

def case_qosApplyGlobalCommandWithHigherStrictLowerWrr():
    with raises(TcamResourcesException):
        with ops1.libs.vtysh.Configure() as ctx:
            ctx.apply_qos_queue_profile_schedule_profile(
                'p1', 'HigherStrictLowerWrr')

def case_qosApplyGlobalCommandWithLowerStrictHigherWrr():
    with raises(TcamResourcesException):
        with ops1.libs.vtysh.Configure() as ctx:
            ctx.apply_qos_queue_profile_schedule_profile(
                'p1', 'LowerStrictHigherWrr')
...
def case_qosApplyPortCommand():
    with ops1.libs.vtysh.ConfigInterface(format('{p1}')) as ctx:
        ctx.apply_qos_schedule_profile('p1')
    out = ops1.libs.vtysh.show_qos_schedule_profile()
    assert out['p1']['profile_status'] == 'applied'
    assert out['p1']['profile_name'] == 'p1'
...
def case_qosApplyPortCommandWithAllWrrWithMaxStrict():
    with ops1.libs.vtysh.ConfigInterface(format('{p1}')) as ctx:
        ctx.apply_qos_schedule_profile('AllWrrWithMaxStrict')
...
def case_qosApplyPortNoCommand():
    ...
    out = ops1.libs.vtysh.show_qos_schedule_profile()
    assert out['p1']['profile_status'] == 'complete'
    assert out['p1']['profile_name'] == 'p1'
