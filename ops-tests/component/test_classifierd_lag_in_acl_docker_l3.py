...
max_no_of_aces = 512
acl_name1 = 'testACL'
acl_name2 = 'maxtestACL'
acl_name3 = 'rollbackACL'
port_id_s2 = 'if01'
...
# Nodes
[type=openswitch name="openswitch 1" image="opsplugin:latest"] ops1
[type=openswitch name="openswitch 2" image="opsplugin:latest"] ops2
...
def teardown_topology(ops1, ops2):
    for ops in [ops1, ops2]:
        ...
        if acl_name1 in run:
            unconfigure_acl(ops, 'ip', acl_name1)
        if acl_name2 in run:
            unconfigure_acl(ops, 'ip', acl_name2)
        if acl_name3 in run:
            unconfigure_acl(ops, 'ip', acl_name3)
...
def test_create_lag_add_members_apply_acl_remove_one_member(topology,
    ...
    # Verify using the command appctl container/show-acl-bindings
    verify_acl_bindings(ops2, acl_name1, interface_list[1:], 'in')
...
def test_create_lag_add_members_apply_acl_delete_lag(topology,
    ...
    verify_acl_bindings(ops2, acl_name1, [], 'in')
...
def test_create_lag_add_members_apply_acl_shut_lag_change_acl(topology,
    ...
    verify_acl_bindings(ops1, acl_name1, [], 'in')
    # Change ACL
    configure_acl_l3(
        ops2, 'ip', acl_name1, str(no_of_aces+1), 'permit', 'udp', '10.0.10.1',
        '', '10.0.10.2', '', '')
    # Verify now acl is applied to no interfaces after shutdown
    verify_acl_bindings(ops2, acl_name1, [], 'in')
    # No Shut LAG
...
def test_lag_members_down_with_pspo(topology):
    ...
    # Apply ACL to a port
    apply_acl(ops2, 'port', port_id_s2, 'ip', acl_name2, 'in')
    # Verify acl is applied to port
    # use command appctl container/show-acl-bindings
    verify_acl_bindings(ops2, acl_name2, [port_id_s2], 'in', True)
    # Configure the switch - Create a lag, add interfaces, configure
    #           routes and assign ip address to lag and interface 1
    config_switches_l3_lag(ops1, ops2, ip_ops1_int1, ip_ops2_int1,
                           ip_ops1_lag, ip_ops2_lag, ip_route_ops1,
...
def test_acl_rollback_on_lag_member(topology):
    ...
    create_acl_with_random_aces(ops2, acl_name1, no_of_aces)
    # Create ACL
    create_acl_with_random_aces(ops2, acl_name3, no_of_aces)
    # Apply ingress ACL
    apply_acl(ops2, 'lag', str(lag_id_s2), 'ip', acl_name1, 'in')
    # print(ops2('show run'))
    # Apply egress ACL
    apply_acl(ops2, 'lag', str(lag_id_s2), 'ip', acl_name3, 'out')
    # Verify now acl entry is applied to lag using
    # appctl container/show-acl-bindings
    verify_acl_bindings(ops2, acl_name1, [interface_list[0]], 'in', True)
    # Verify now acl entry is applied all interfaces using
    # appctl container/show-acl-bindings
    verify_acl_bindings(ops2, acl_name3, [interface_list[0]], 'out', True)
    # Add a member to LAG
    ...
    verify_acl_bindings(ops2, acl_name1, [interface_list[1]], 'in', False)
    verify_acl_bindings(ops2, acl_name3, [interface_list[1]], 'out', False)
