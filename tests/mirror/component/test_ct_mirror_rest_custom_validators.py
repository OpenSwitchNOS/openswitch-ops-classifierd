...
class Test_mirror_rest_custom_validators:
    ...
    def test_1_activate_ms_foo_succeeds(self):
        ...
        assert "\"select_src_port\": [\"/rest/v1/system/ports/2\"]," \
            in actual_data
        assert "\"select_dst_port\": [\"/rest/v1/system/ports/2\"]" \
            in actual_data
        assert "output_port\": [\"/rest/v1/system/ports/3\"]" in actual_data
    ...
    def test_2_add_second_source_to_active_ms_foo_succeeds(self):
        ...
        actual_data = self.rest_get_succeeds(mirror_name)
        assert "\"name\": \"foo\"" in actual_data
        assert "\"select_src_port\": [\"/rest/v1/system/ports/1\","
        " \"/rest/v1/system/ports/2\"]," in actual_data
        assert "\"select_dst_port\": [\"/rest/v1/system/ports/2\"]" \
            in actual_data
        assert "output_port\": [\"/rest/v1/system/ports/3\"]" in actual_data
        assert "\"active\": true" in actual_data
    ...
    def test_6_activate_ms_bar_succeeds(self):
        ...
        assert "\"select_dst_port\": [\"/rest/v1/system/ports/1\"]" \
            in actual_data
        assert "output_port\": [\"/rest/v1/system/ports/4\"]" in actual_data
        assert "\"active\": true" in actual_data
